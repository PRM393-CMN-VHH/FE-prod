class UserModel {
  ...
  factory UserModel.fromJson(Map<String, dynamic> json) {
    ...
    String parsedRoleName = json['roleName'] ?? json['role']?.toString() ?? 'user';
    int? parsedRoleId;
    if (json['role'] is Map) {
      final role = json['role'] as Map<String, dynamic>;
      parsedRoleName = role['roleName'] ?? parsedRoleName;
      parsedRoleId = role['roleId'] is int ? role['roleId'] as int : int.tryParse(role['roleId']?.toString() ?? '');
    } else {
      parsedRoleId = json['roleId'] is int ? json['roleId'] as int : int.tryParse(json['roleId']?.toString() ?? '');
    }
