...
import 'package:prm393/utils/order_status_formatter.dart';
                    ...
                    child: ListTile(
                      onTap: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) =>
                                OrderDetailScreen(orderId: orderId as int),
                          ),
                        );
                      },
                      title: Text(
                        "Đơn #$orderId - ${orderStatusLabel(currentStatus.toString())}",
                      ),
                      subtitle: Text(
                ...
                itemBuilder: (_, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                        "${product.hasDiscount ? '${formatVnd(product.promoPrice!)} (${product.discountPercent}% giảm) - Giá gốc: ${formatVnd(product.price)}' : formatVnd(product.price)} - Tồn: ${product.stock}"
                        "${product.color.isEmpty ? '' : ' - Màu: ${flowerColorLabel(product.color)}'}",
                      ),
                      trailing: Wrap(
                        children: [
                          IconButton(
...
class _ProductEditorDialogState extends State<_ProductEditorDialog> {
  ...
  void initState() {
    ...
    if (product != null) {
      ...
      _promoPriceController.text = product.promoPrice == null
          ? ''
    ...
    ApiService().getProducts().then((products) {
      ...
      final colors = products
          .map((product) => flowerColorLabel(product.color))
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (value) {
                  ...
                  final promoPrice = double.tryParse(text);
                  final price = double.tryParse(_priceController.text.trim());
                  if (promoPrice == null) return "Giá khuyến mãi không hợp lệ";
                  if (price != null && promoPrice >= price) {
                    return "Giá khuyến mãi phải nhỏ hơn giá gốc";
                  }
                  if (promoPrice <= 0) return "Giá khuyến mãi phải lớn hơn 0";
                  return null;
                },
              ),
              TextFormField(
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: "Màu hoa",
                  hintText: "Ví dụ: Đỏ, Trắng, Hồng",
                  helperText: _knownColors.isEmpty
                      ? null
                      : "Chọn màu có sẵn hoặc nhập màu mới",
                  suffixIcon: _knownColors.isEmpty
                      ? null
                      : PopupMenuButton<String>(
                          icon: const Icon(Icons.arrow_drop_down),
                          tooltip: "Chọn màu hiện có",
                          onSelected: (color) {
                            setState(() {
                              _colorController.text = color;
                            });
                          },
                          itemBuilder: (_) => _knownColors
                              .map(
